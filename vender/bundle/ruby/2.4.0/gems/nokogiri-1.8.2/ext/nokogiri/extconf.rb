...
def sh_export_path(path)
  ...
  if windows?
    ...
    return File.join('/', match[1], match[2]) if match && match.length == 3
  end
  path
end
...
def package_config(pkg, _options = {})
...
def iconv_configure_flags
  ...
  return ['--with-iconv=yes'] if have_iconv?
...
def process_recipe(name, version, static_p, cross_p)
  MiniPortile.new(name, version).tap do |recipe|
    ...
    env = Hash.new do |hash, key|
      hash[key] = (ENV[key]).to_s # (ENV[key].dup rescue '')
    end
...
$CFLAGS << ' -DXP_WIN -DXP_WIN32 -DUSE_INCLUDED_VASPRINTF' if windows?

$CFLAGS << ' -DUSE_INCLUDED_VASPRINTF' if solaris?
      ...
      File.join(recipe.path, 'bin', "#{libname}-config").tap do |config|
        ...
        `sh #{config} --libs`.strip.shellsplit.each do |arg|
          ...
          when /\A-L(.+)\z/
            ...
            $LIBPATH = if Regexp.last_match(1).start_with?(ROOT + '/')
